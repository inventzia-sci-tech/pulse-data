...
class EngineStatus(BaseModel):
    ...
    component: str
    """Name of the component whose status changed — the engine, or one of its gateways (e.g. "Engine", "Beater-5s")"""
    changed_at: int = Field(alias="changedAt")
    """Wall-clock time of the transition, epoch milliseconds. Wall clock rather than simulation time because a lifecycle change is a fact about the process, not about the data being replayed"""
    from_status: str = Field(alias="fromStatus")
    """The status the component left (its previous GatewayStatus name)"""
    to_status: str = Field(alias="toStatus")
    """The status the component entered (its new GatewayStatus name)"""
